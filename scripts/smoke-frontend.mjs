...
check('activity: escaping, thumbnail helper, translated failure reason', () => {
  ...
  assert.equal(rows[0].querySelector('.row-info .text-primary').textContent, 'A $$ "quoted" & <b>')
  assert.ok(rows[1].querySelector('.row-info .text-secondary').textContent.length > 0)
